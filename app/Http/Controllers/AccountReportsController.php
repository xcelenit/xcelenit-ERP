...
class AccountReportsController extends Controller
{
    ...
    public function paymentAccountReport()
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($query)
                    ...
                    ->addColumn('action', function ($row) {
                        ...
                        return $action;
                    })->addColumn('select_pmt', function ($row) {
                        return  '<input type="checkbox" class="row-select" value="' . $row->payment_id .'">' ;
                    })
                    ...
                    ->rawColumns(['action', 'transaction_number','select_pmt'])
                    ->make(true);
        }
        ...
        return view('account_reports.payment_account_report')
                ->with(compact('accounts','accounts_list'));
    }
    ...
    public function postLinkAccountBulk(Request $request)
    {
        if (!auth()->user()->can('account.access')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            ...
            if (request()->ajax()) {
                $payment_id_list = $request->input('payment_id_list');
                $account_id = $request->input('account_id');
               // return json_encode($payment_id_list);
                DB::beginTransaction();
               // return $account_id;
                foreach($payment_id_list as $pmt_id){
                                      

                    $payment = TransactionPayment::with(['transaction'])->where('business_id', $business_id)->findOrFail($pmt_id);
                    $payment->account_id = $account_id;
                    $payment->save();
