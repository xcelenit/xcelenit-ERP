...
class Util
{
    ...
    */
    public function numToWord($number, $lang = null)
    {
        ...
        $number_ft =number_format($number,2,'.','');
         
        $number_explode = explode('.',$number_ft);
        

        $lang = empty($lang) ? auth()->user()->language : $lang;
        $f = new \NumberFormatter($lang, \NumberFormatter::SPELLOUT);
        
        $number_in_word = $f->format($number_explode[0]);

        if(($number_explode[1])>0){

            $number_in_word .=' and '.$f->format($number_explode[1]).' cents';
        }
