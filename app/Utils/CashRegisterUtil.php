...
class CashRegisterUtil extends Util
{
    ...
    {   
        ...
        $sells_return = $this->getReturnDetails($register_details->location_id, $register_details->open_time, $register_details->closed_at, $register_details->user_id);
    ...
    public function getReturnDetails($location_id, $start, $end, $created_by)
    {   

        \Log::emergency('This Mine '.$start.' = '.$end);
      ...
      //  $sells->where('transactions.created_by', $created_by);
        ...
        //     $end =  request()->end_date;
            $sells->whereBetween(DB::raw('DATE(transactions.transaction_date)'), [date('Y-m-d',strtotime($start)), date('Y-m-d', strtotime($end))]);
            // $sells->whereBetween(DB::raw('DATE(transactions.transaction_date)'), [$start, $end])
                    //->where(DB::raw('DATE(transactions.transaction_date)'), '<=', date('Y-m-d', strtotime($end)))
        // }

       
        // ->groupBy('transactions.id');
        //->get();
        //\Log::emergency($sellsdata);
        $sellsdata = $sells->sum('transactions.final_total');
        //transactions.final_total
