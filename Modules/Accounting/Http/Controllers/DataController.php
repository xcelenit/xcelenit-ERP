...
class DataController extends Controller
{
       ...
       // \Log::emergency($data['transaction']);
        ...
        foreach($transaction_details->payment_lines as $payment_line){
            ...
            if($payment_line->is_return==0){
                ...
                $total_amt = $payment_line->amount;

                

                if(($total_paid_rows+$payment_line->amount) > $transaction_details->final_total){

                    $total_amt = $transaction_details->final_total - $total_paid_rows;
                }
    ...
    public function after_purchases_saved_acc($data)
    {
        ...
        $transaction=[
            ...
            'transaction_date'=>$transaction_data['transaction_date'],
            'total_amount'=>$transaction_data['final_total'],
            'purchase_ref'=> $transaction_data['ref_no'],
            'supplier_invoice_no'=>$transaction_data['staff_note'], 
    ...
    public function after_debit_note_saved_acc($data)
    {
        if(!(config('accounting.link_transaction.debit_note'))){
            return null;
        }
       ...
       //\Log::emergency($data['transaction']);
        ...
        $transaction_details = Transaction::with('payment_lines')                                  
                            ->where('transactions.type','purchase_return')
                            // ->whereIn('transactions.id',[3965, 3966, 3967])
                            ->where('transactions.id',$transaction_data['id'])
      ...
      // \Log::emergency($transaction_details);
        ...
        $transaction=[
            ...
            'transaction_date'=>$transaction_details->transaction_date,
            'total_amount'=>$transaction_details->final_total,
            'purchase_ref'=>$transaction_details->ref_no,
            'total_cost'=> 0,
            'sys_transaction_id'=>$transaction_details->id,
            'transaction_id'=>$transaction_id,
        ];
