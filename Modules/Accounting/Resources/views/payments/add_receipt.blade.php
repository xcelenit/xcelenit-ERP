...
<section class="content">
    @component('components.widget', ['class' => 'box-solid','title'=> isset($transaction) ? 'Edit Receipt' : 'Add New' ])
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <table class="custom_table">
                <tbody>
                    ...
                    <tr>                     
                        ...
                        <td colspan="3" class="col-sm-6">
                            {!! Form::select('vendor_id', $contact_dropdown, isset($transaction) ? $transaction->vendor_id : null, ['id' => 'vendor_id' ,'class' => 'form-control select2', 'required']); !!}
            ...
            <table id="cheque_data_table" class="custom_table @if(isset($transaction))  @if($transaction->payment_method != "cheque") hide @endif  @endif">
                <tbody>                   
                    <tr>
                        <td class="col-sm-2 text-left">
                        ...
                        <td class="col-sm-2">                            
                            {!! Form::text('cheque_no', isset($transaction) ? $transaction->cheque_no : null, ['placeholder' => '', 'id'=>'cheque_no', 'class' => 'form-control','required']); !!}
            ...
            <table class="custom_table">
                <tbody>
                    <tr>
                        ...
                        <td class="col-sm-6" >                           
                            {!! Form::select('account_id', $debit_account_list, isset($transaction) ? $transaction->debit_account_id : null, ['id' => 'account_id' ,'class' => 'form-control select2', 'required']); !!}
                ...
                <tbody>                   
                    <tr>
                        ...
                        <td style="padding: 5px;" class="col-sm-2">  
                            <button type="button" style="margin-left: 10px" onclick="storeData()" class="btn btn-primary pull-right">Submit</button>
                            <button type="button" onclick="storeData()" class="btn btn-success pull-right">Submit & Print</button> 
                        </td>                                                
                    </tr>                  
                </tbody>
            </table> 
    ...
    $('#cheque_date').datetimepicker({
          ...
          var isValidated = validatingData();
           ...
           if($('#error_in_ad').val()==1){
              ...
              if(parseFloat(total_adjusted_val)!=parseFloat(amount)){
                ...
                $.ajax({
                    method: 'POST',
                    url: '/accounting/payment/debtor/store',
                    // dataType: 'JSON',
                    data: { 
                        transaction_id: transaction_id,
                        vendor_id: vendor_id , transaction_date: transaction_date,                        
                        payment_method: payment_method, cheque_no: cheque_no,
        ...
        function checkAdjustment(){
            $('#inv_list_body tr').each(function() {
                ...
                var isAdjusted = $(this).find(".transaction_id").prop('checked'); 
                var total_adjusted = parseFloat($('#total_adjusted').val());
                var total_paying = parseFloat($('#amount').val());
              //  console.log(total_adjusted)
