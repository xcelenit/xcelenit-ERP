<tr class="product_row" data-row_index="{{$row_count}}">
	<td>
		...
		@endphp
		<input type="hidden" class="product_sku" value="{{$product->sub_sku}}">
	...
	<td>
		...
		<div class="input-group input-number">
			...
			@if ($product->enable_sr_no==1)
				Serial NO
				<input type="text" class="form-control product_serial_no" name="products[{{$row_count}}][product_serial_no]" required>
			@endif
			
			@if(($location_id_df==2) && ($is_pg==false))
				<input type="hidden" class="pos_disc_quantity" value="{{$product->product_custom_field2}}">
				<input type="hidden" class="pos_disc_quantity_val" value="{{$product->product_custom_field3}}">
			@else
				<input type="hidden" class="pos_disc_quantity" value="0">
				<input type="hidden" class="pos_disc_quantity_val" value="0">
			@endif
	...
	@endif
	<td class="{{$hide_tax}}">
		<input type="text" name="products[{{$row_count}}][unit_price_inc_tax]" class="form-control pos_unit_price_inc_tax input_number text-right" value="{{@num_format($unit_price_inc_tax)}}" @if(!$edit_price) readonly @endif @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$unit_price_inc_tax}}" data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($unit_price_inc_tax)])}}" @endif>
		Discount 
		<input type="text" disabled class="form-control input_number pos_disc_lbl text-right" value="0.00">
	</td>	
