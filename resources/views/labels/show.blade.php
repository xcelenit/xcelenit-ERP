	...
	@component('components.widget', ['class' => 'box-primary', 'title' => __('product.add_product_for_labels')])
		...
		@if(!empty($supplier))
			<div class="row">
				...
				<h4 class="box-title text-info">#Ref No : {{$supplier->ref_no}} &nbsp; -  &nbsp; Supplier Name : {{$supplier->supplier_business_name}} - {{$supplier->name}} ({{$supplier->contact_id}}) / {{$transaction_date}}</h4>
				</div>
			</div>
			{!! Form::hidden('contact_id', $supplier->contact_id.'/'.$transaction_date) !!}
		@endif
